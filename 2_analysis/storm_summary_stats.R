...
# =============================================================================
storm_tx_by_month <- storm_tx %>%
  count(month, month_num, cat_fac, max_cat) %>%
  arrange(month_num)
...
# =============================================================================
storm_tx_by_season <- storm_tx %>%
  count(year, cat_fac, max_cat, name) %>%
  arrange(year) %>%
  mutate(year = as.numeric(as.character(year))) 

storm_tx_by_season %>%
  filter(max_cat > 0) %>%
  mutate(year = ifelse(name == "JERRY", year - 0.25, year),
         year = ifelse(name == "CHANTAL", year + 0.25, year)) %>%
  ggplot(aes(x = year, y = cat_fac, color = cat_fac, label = name)) +
...
storm_tx_by_season %>% 
  subset(max_cat>0) %>%
  ggplot(aes(x = year, y = n, fill=cat_fac)) +
...
storm_tx_by_season %>% 
  ggplot(aes(x = year, y = n, fill=cat_fac)) +
  geom_col(linewidth = 0.4) +
  labs(x = "year", y = "Number of storm_tx") +
  graph$hurr_theme + 
...
exposures_per_county <- school_storm_unique %>%
  ...
  summarise(wind_50kt = any(wind_50kt)) %>%
  group_by(county_fips) %>%
  summarise(n_storm_tx = sum(wind_50kt)) 
